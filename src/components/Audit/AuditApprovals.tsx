...
const COLUMN_DEFINITIONS = [
  ...
  },
  {
    ...
    header: "Approver",
    cell: (item: RequestItem) => item.approver || "-",
    minWidth: 10,
  },
  {
    id: "status",
...
function AuditApprovals(props: AuditApprovalsProps) {
  ...
  // Derive data from query - normalize status for display
  const allItems = useMemo(() => {
    const items = requestsQuery.data?.items as unknown as RequestItem[] | undefined;
    if (!items) return [];
