...
# The pipeline's deploy stage reads these at run time (see cdk/lib/pipeline-stack.ts),
# so changing a value here and re-running takes effect on the next pipeline run —
# no pipeline redeploy.
#
# Usage: ./config-put.sh <env>        # values are read from 00-params.sh
#
...
# Load values from 00-params.sh unless they're already exported in the environment.
